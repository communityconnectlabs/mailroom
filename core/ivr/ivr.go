...
func RequestCallStartForConnection(ctx context.Context, rt *runtime.Runtime, channel *models.Channel, telURN urns.URN, conn *models.ChannelConnection, isRetry bool) error {
	...
	oa, err := models.GetOrgAssetsWithRefresh(ctx, rt, conn.OrgID(), models.RefreshOrg)
...
func HandleIVRStatus(ctx context.Context, rt *runtime.Runtime, oa *models.OrgAssets, svc Service, conn *models.ChannelConnection, r *http.Request, w http.ResponseWriter) error {
	...
	if status == models.ConnectionStatusErrored {
		...
		err = models.NewEmptyRun(ctx, rt.DB, contactID, flow.ID(), oa.OrgID())
		if err != nil {
			return errors.Wrapf(err, "error creating empty run")
		}

		conn.MarkBusy(ctx, rt.DB, time.Now())
	} else {
