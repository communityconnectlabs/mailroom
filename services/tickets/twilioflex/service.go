...
func NewService(rtCfg *config.Config, httpClient *http.Client, httpRetries *httpx.RetryConfig, ticketer *flows.Ticketer, config map[string]string) (models.TicketService, error) {
	...
	if authToken != "" && accountSid != "" && chatServiceSid != "" && workspaceSid != "" {
		...
		return &service{
			client:   NewClient(httpClient, httpRetries, authToken, accountSid, chatServiceSid, workspaceSid, flexFlowSid),
			ticketer: ticketer,
...
func (s *service) Open(session flows.Session, subject, body string, logHTTP flows.HTTPLogCallback) (*flows.Ticket, error) {
	...
	contactUser, trace, err := s.client.FetchUser(chatUser.Identity)
	...
	if contactUser == nil {
		_, trace, err := s.client.CreateUser(chatUser)
		if trace != nil {
			logHTTP(flows.NewHTTPLog(trace, flows.HTTPStatusFromCode, s.redactor))
		}
		if err != nil {
	...
	flexChannelParams := &CreateFlexChannelParams{
		FlexFlowSid:          s.client.flexFlowSid,
		Identity:             fmt.Sprint(contact.ID()),
	...
	newFlexChannel, trace, err := s.client.CreateFlexChannel(flexChannelParams)
	if trace != nil {
		logHTTP(flows.NewHTTPLog(trace, flows.HTTPStatusFromCode, s.redactor))
	}
	if err != nil {
	...
	_, trace, err = s.client.CreateFlexChannelWebhook(channelWebhook, newFlexChannel.Sid)
	if trace != nil {
		logHTTP(flows.NewHTTPLog(trace, flows.HTTPStatusFromCode, s.redactor))
	}
	if err != nil {
...
func (s *service) Forward(ticket *models.Ticket, msgUUID flows.MsgUUID, text string, attachments []utils.Attachment, logHTTP flows.HTTPLogCallback) error {
	...
	if len(attachments) > 0 {
		...
		for _, attachment := range attachments {
			...
			resp, err := httpx.DoTrace(s.client.httpClient, req, s.client.httpRetries, nil, -1)
			if err != nil {
				return err
			}
		...
		for _, mediaParams := range mediaAttachements {
			media, trace, err := s.client.CreateMedia(&mediaParams)
			if err != nil {
				return err
			}
			logHTTP(flows.NewHTTPLog(trace, flows.HTTPStatusFromCode, s.redactor))
...
func (s *service) Close(tickets []*models.Ticket, logHTTP flows.HTTPLogCallback) error {
	for _, t := range tickets {
		flexChannel, trace, err := s.client.FetchFlexChannel(string(t.ExternalID()))
		if trace != nil {
			logHTTP(flows.NewHTTPLog(trace, flows.HTTPStatusFromCode, s.redactor))
		}
		if err != nil {
		...
		_, trace, err = s.client.CompleteTask(flexChannel.TaskSid)
		if trace != nil {
			logHTTP(flows.NewHTTPLog(trace, flows.HTTPStatusFromCode, s.redactor))
		}
		if err != nil {
