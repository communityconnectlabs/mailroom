...
func (c *Client) CreateFlexChannel(channel *CreateFlexChannelParams) (*FlexChannel, *httpx.Trace, error) {
	url_ := "https://flex-api.twilio.com/v1/Channels"
	...
	trace, err := c.post(url_, data, response)
	if err != nil {
		return nil, trace, err
	}
...
func (c *Client) CreateMessage(message *CreateChatMessageParams) (*ChatMessage, *httpx.Trace, error) {
	url_ := fmt.Sprintf("https://chat.twilio.com/v2/Services/%s/Channels/%s/Messages", c.serviceSid, message.ChannelSid)
...
func (c *Client) CompleteTask(taskSid string) (*TaskrouterTask, *httpx.Trace, error) {
	url_ := fmt.Sprintf("https://taskrouter.twilio.com/v1/Workspaces/%s/Tasks/%s", c.workspaceSid, taskSid)
...
func (c *Client) CreateMedia(media *CreateMediaParams) (*Media, *httpx.Trace, error) {
	url_ := fmt.Sprintf("https://mcs.us1.twilio.com/v1/Services/%s/Media", c.serviceSid)
	response := &Media{}
	...
	req, err := httpx.NewRequest("POST", url_, body, map[string]string{})
	if err != nil {
		return nil, nil, err
	}
	req.SetBasicAuth(c.accountSid, c.authToken)
